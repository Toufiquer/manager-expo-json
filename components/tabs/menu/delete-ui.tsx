...
import { menuItemType, storeType } from "@/types/schemas/menu";
    ...
    if (currentUIData.item) {
      for (let m in menu) {
        const checked =
          ...
          currentUIData.title.split("-").join(" ").split("_").join(" ").toLocaleLowerCase();
        if (checked) {
          result[m] = {
            srl: menu[m]?.srl,
            available: menu[m]?.available,
            lst: menu[m]?.lst?.filter((curr) => curr.item !== currentUIData.item),
          };
          if (menu[m]?.i) {
            result[m].i = menu[m].i;
          }
        ...
        <View className="relative h-full w-full">
          <View className="flex w-full">
            <View className="flex w-full flex-row items-end justify-between">
              <Text style={styles.fontsMulishBlack} className="max-w-[80%] text-2xl font-extrabold text-gray-900">
                {currentUIData.item?.split("_").join(" ") || currentUIData.title?.split("_").join(" ")}
              </Text>
          ...
          <View className="absolute bottom-[250px] self-center ">
            <View className="mt-6 w-full  p-6">
              <View className="flex flex-row items-center justify-between">
                <Text style={styles.fontsMulishBlack} className="text-2xl font-bold text-rose-500">
                  Note
                </Text>
              </View>
              <View className="flex flex-row items-center justify-between">
          ...
          <View className="absolute bottom-[-100px] w-full items-center justify-end self-center pb-[200px]">
            <Text style={styles.fontsMulishBlack} className="text-center text-2xl text-rose-400">
              Are You Confirm?
            </Text>
            <TouchableOpacity
              disabled={process}
            ...
            >
              <View className="flex flex-row items-center justify-center gap-1">
                <Text style={styles.fontsMulishBlack} className="p-2 text-center text-xl font-semibold text-white">
                  Delete
                </Text>
